...
FullLimitMultyThreadPowerOptimisationHGSAScreen::FullLimitMultyThreadPowerOptimisationHGSAScreen(){
	this->apply();
}


void FullLimitMultyThreadPowerOptimisationHGSAScreen::printHead() {
	print("----  Simulated annealing based on Hooke - Jeeves multy-thread optimisation of input power ----\n");
	print("----                                      Omsk, 2018                                ----\n");
	print("\n");
}

FAROptimisationAlgoritm *FullLimitMultyThreadPowerOptimisationHGSAScreen::getAgoritm(OptimisationEnvirounment *envirounment) {
	...
	return new FullLimitMultyThreadPowerOptimisationHGSAAlgoritm(envirounment, P, T, speed, radius, mulctMultiplier, mulctDegree, threadsCount, randomStartRadius);
}
