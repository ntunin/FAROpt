...
AllSourceNecInWritter::AllSourceNecInWritter(ComplexVector *sourceValues, NecIn *in, std::string name) {
	try {
		this->name = name;
...
void AllSourceNecInWritter::write(ofstream &file, vector<EX *> *commandsPointer, ComplexVector *sources) {
	if (commandsPointer) {
